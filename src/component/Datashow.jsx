  ...
  return (
    ...
    {filteredItems.map((item) => (
        ...
        <button className='wishlist__button' onClick={() => toggleWishlist(item)}>
          {
            wishlist.some((wishedItem) => wishedItem.id === item.id) ? <img src={"/assets/heart-red.svg"} alt="wishlist_red" width={"20px"} className='button_red' /> : <img src={"/assets/heart.svg"} alt="wishlist_black" width={"20px"} />
          }
        </button>
        </div>
      ))}
